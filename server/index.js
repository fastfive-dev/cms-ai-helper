...
## 설명에서 제외할 자명한 UI 기능
다음과 같이 사용자가 눈으로 보고 바로 이해할 수 있는 범용 UI 컨트롤은 기능 설명에 포함하지 마세요 (사용자가 해당 기능을 명시적으로 질문한 경우에만 설명):
- 페이지네이션 컨트롤 (◀/▶ 이전/다음 버튼, 페이지 번호)
- 한 페이지 행 수 설정 (Rows per page, 페이지 크기)
- 테이블 컬럼 정렬 (컬럼 헤더 클릭)
- 일반적인 검색창, 새로고침 버튼, 닫기(×) 버튼
- 브라우저/OS 기본 기능 (스크롤, 뒤로가기 등)
해당 화면 고유의 업무 로직, 상태 전환, 데이터 입력/승인 흐름, 도메인 용어 설명에 집중하세요.
${knowledgeContent ? "\n## 상세 사용 가이드\n" + knowledgeContent : ""}`;
