// popup.js — Communicates with background service worker for auth operations.

const elements = {
  loading: document.getElementById('loading'),
  loginSection: document.getElementById('loginSection'),
  userSection: document.getElementById('userSection'),
  loginBtn: document.getElementById('loginBtn'),
  logoutBtn: document.getElementById('logoutBtn'),
  errorMessage: document.getElementById('errorMessage'),
  userName: document.getElementById('userName'),
  userEmail: document.getElementById('userEmail'),
  userAvatar: document.getElementById('userAvatar'),
};

const GOOGLE_ICON_SVG = `
  <svg viewBox="0 0 24 24" fill="currentColor" width="18" height="18">
    <path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92a5.06 5.06 0 0 1-2.2 3.32v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.1z" fill="#4285F4"/>
    <path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853"/>
    <path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" fill="#FBBC05"/>
    <path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#EA4335"/>
  </svg>`;

function showLoading() {
  elements.loading.style.display = 'block';
  elements.loginSection.style.display = 'none';
  elements.userSection.style.display = 'none';
}

function showLoginView(errorText) {
  elements.loading.style.display = 'none';
  elements.loginSection.style.display = 'block';
  elements.userSection.style.display = 'none';

  if (errorText) {
    elements.errorMessage.textContent = errorText;
    elements.errorMessage.style.display = 'block';
  } else {
    elements.errorMessage.style.display = 'none';
  }
}

function showUserView(user) {
  elements.loading.style.display = 'none';
  elements.loginSection.style.display = 'none';
  elements.userSection.style.display = 'block';

  elements.userName.textContent = user.name || 'User';
  elements.userEmail.textContent = user.email || '';

  if (user.picture) {
    elements.userAvatar.src = user.picture;
    elements.userAvatar.style.display = 'block';
  } else {
    elements.userAvatar.style.display = 'none';
  }
}

async function initialize() {
  showLoading();

  try {
    const response = await chrome.runtime.sendMessage({ type: 'auth_status' });
    if (response && response.isAuthenticated && response.user) {
      showUserView(response.user);
    } else {
      showLoginView();
    }
  } catch {
    showLoginView();
  }
}

elements.loginBtn.addEventListener('click', async () => {
  elements.loginBtn.disabled = true;
  elements.loginBtn.textContent = '로그인 중...';
  elements.errorMessage.style.display = 'none';

  try {
    const response = await chrome.runtime.sendMessage({ type: 'auth_login' });

    if (response && response.success) {
      showUserView(response.user);
    } else {
      const errorText = (response && response.error)
        ? response.error
        : '로그인에 실패했습니다. 다시 시도해주세요.';
      showLoginView(errorText);
    }
  } catch {
    showLoginView('예기치 않은 오류가 발생했습니다. 다시 시도해주세요.');
  } finally {
    elements.loginBtn.disabled = false;
    elements.loginBtn.innerHTML = `${GOOGLE_ICON_SVG} Google 계정으로 로그인`;
  }
});

elements.logoutBtn.addEventListener('click', async () => {
  elements.logoutBtn.disabled = true;
  elements.logoutBtn.textContent = '로그아웃 중...';

  try {
    await chrome.runtime.sendMessage({ type: 'auth_logout' });
    showLoginView();
  } catch {
    showLoginView();
  } finally {
    elements.logoutBtn.disabled = false;
    elements.logoutBtn.textContent = '로그아웃';
  }
});

initialize();
